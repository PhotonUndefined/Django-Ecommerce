...
def process_order(request):
    if request.method == "POST":
        ...
        # Get Shipping Sesson Data
        my_shipping_info = request.session.get('my_shipping_info')
        
        # Gather Order Info from Shipping Session
...
def billing_info(request):
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            ...
            return render(request, "payment/billing_info.html", {"paypal_form":paypal_form, "cart_products":cart_products, "quantities": quantities, "totals": totals, "shipping_info": request.POST or None, "billing_form": billing_form})
        else:
            billing_form = PaymentForm(request.POST or None)
            return render(request, "payment/billing_info.html", {"paypal_form":paypal_form, "cart_products":cart_products, "quantities": quantities, "totals": totals, "shipping_info": request.POST or None, "billing_form": billing_form})
        
        shipping_form = request.POST or None
